...
/* ── Instrument-Gruppe ──────────────────────────────────────────── */
function InstrumentGroup({ instrument, belegeForInstrument, onAddBeleg }) {
  return (
    <div className="mb-6">
      <div className="flex items-center justify-between mb-2">
        ...
        {WEBHOOK_BELEG_UPLOAD && (
          <button
            ...
            className="inline-flex items-center gap-1 text-xs text-orange-400 bg-orange-500/10 border border-orange-500/30 px-2.5 py-1 rounded-lg hover:bg-orange-500/20 transition-all"
          >
      ...
      {belegeForInstrument.length === 0 ? (
        ...
        belegeForInstrument.map((b) => (
          <BelegCard key={b.id} beleg={b} instrument={instrument} />
        ))
      )}
    </div>
  );
...
export default function EinkaufView({ data, reload }) {
  ...
  const instrumentsWithBelege = useMemo(() => {
    // Alle Instrument-IDs, die Belege haben
    ...
    // Aktive Instrumente (nicht ausgemustert), die Belege haben
  ...
  /* Flow: 1) POST neue Row → 2) Upload Datei → 3) PATCH URL */
  const handleSaveBeleg = async (form, file) => {
    setSaving(true);
    try {
      const instrument = modalInstrument;

      // 1. Neue Baserow Row erstellen
      ...
      // 2. Datei hochladen (wenn vorhanden)
      if (file && WEBHOOK_BELEG_UPLOAD) {
        const ext = file.name.split(".").pop();
        const dateiname = `Beleg-${newRow.Beleg_ID || newRow.id}_${(form.Händler || "").replace(/\s/g, "-")}_${form.Kaufdatum || "unbekannt"}.${ext}`;

        const formData = new FormData();
        formData.append("file", file);
        formData.append("beleg_row_id", String(newRow.id));
        formData.append("instrument_id", String(instrument.Instrument_ID));
        ...
        if (driveUrl) {
          // 3. URL in die NEUE Row schreiben
          await updateRow(TABLE_IDS.belege, newRow.id, { Beleg_URL: driveUrl });
        }
      }

      setToast({ message: "Einkaufsbeleg gespeichert", type: "success" });
